...
class Elimination(object):
    ...
    def _update(self):
        self.reward_estimates = [self.total_reward[k]/self.pulls[k]
                                 for k in range(self.K)]
...
class Env(object):
    ...
    def pull_arm(self, k):
        return np.random.normal(thetastar[k], self.stddev)
...
if __name__ == "__main__":
    #thetastar = [x/16 for x in list(range(1,16))]
    #readfromfile = False
    #if readfromfile:
    #    thetastar = np.loadtxt('thetastar.txt')
    #    #thetastar = 0.5+(thetastar-0.5)/3
    #else:
    #    thetastar = np.random.random(15)
    #    #thetastar = 0.5+(thetastar-0.5)/2
    #    np.savetxt('thetastar.txt', thetastar)

    K = 100; alpha = 0.6
    gaps = [1]+[1-(i/K)**alpha for i in range(1,K+1)]
    #gaps = [1.2] + [.2]*2 + [1.1] + [.2]*96
    #gaps = [1.5] + [.5]*2 + [1.0] + [.5]*96
    #gaps = [1.2] + [.5]*2 + [1.1] + [.5]*96
    #thetastar = np.cumsum(gaps[::-1])[::-1]
    thetastar = np.array(pd.read_csv('borda.csv',header=None)).flatten()
    thetastar = sorted(thetastar)[10:]
    K = len(thetastar)
    sorted_index = np.argsort(thetastar)[::-1]
    sorted_scores = np.sort(thetastar)[::-1]
    max_gap = max([(i, sorted_scores[i]-sorted_scores[i+1])
...
# plot the means
    ...
    # algorithms = ['Random', 'Elimination', 'GapUCB']
